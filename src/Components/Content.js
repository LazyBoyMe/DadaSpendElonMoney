...
const SellButton=styled.button`
  ...
  background-color: white;
  border: none;
  background-color: ${props=>props.isActive?'#d63031':'rgba(220,220,220)'};
  border-radius: 2px;
  color: white;
  font-weight: 700;
...
const BuyButton=styled.button`
  ...
  background-color: white;
  border: none;
  background-color:${props=>props.isActive?'#a29bfe':'rgba(220,220,220)'} ;
  border-radius: 2px;
  color: white;
  font-weight: 700;
  box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;
    ...
    <Container>
        ...
        <ProductsContainer>
          {arr.map((p,id)=>
            <ProductContainer key={id}>
              ...
              <ProductFooter>
                <SellButton isActive={p.countInCart>0} onClick={()=>p.countInCart>0?dispatch(sellProduct(id)):''}>Sell</SellButton>
                <ProductCountInput  type='number' value={p.countInCart} onChange={(e)=>change(id,p.countInCart,p.price,e)} />
                <BuyButton isActive={money/p.price>=1} onClick={()=>money/p.price>=1?dispatch(buyProduct(id)):''}>Buy</BuyButton>
              </ProductFooter>
            </ProductContainer>
          )}
        </ProductsContainer>
        { money!==187000000000 &&
          <ReceiptContainer>
                ...
                <ReceiptList>
                  {arr.map((p,id)=>{
                    return p.countInCart>0 && 
                    <ReceiptItem key={id}>
                      <ReceipItemText>{p.name}</ReceipItemText>
                      <ReceipItemText align='center'>{p.countInCart}</ReceipItemText>
                      <ReceipItemText align='right'>${numberWithCommas(p.countInCart*p.price)}</ReceipItemText>
                    </ReceiptItem>
                  })}
                  <Line/>
                  <ReceiptItem >
                    <ReceipItemText style={{width:'50%',textAlign:'center'}}>Total</ReceipItemText>
                    <ReceipItemText style={{width:'50%',textAlign:'center'}}>${numberWithCommas(187000000000-money)}</ReceipItemText>
                  </ReceiptItem>
                </ReceiptList>
          </ReceiptContainer>
        }
